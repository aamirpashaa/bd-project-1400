...
query = "CREATE TABLE IF NOT EXISTS channels ( channel_id text, channel_name text static, ts float, post_id text, primary key(channel_id, ts, post_id)) WITH CLUSTERING ORDER BY (ts DESC, post_id DESC);"
result = session.execute(query)
...
insert_channels_query = "INSERT INTO channels (channel_id, channel_name, ts, post_id) VALUES (%s, %s, %s, %s)"
...
while(True):
	for item in consumer:
		...
		session.execute(insert_channels_query, (str(new_item['peer_id']['channel_id']), str(new_item['sender_name']), new_item['timestamp'], new_item['UUID']))
		...
		print(datetime.datetime.now(),"new message inserted into statistics")